...
if (basename($_SERVER['SCRIPT_FILENAME']) == 'showmap.php') { die ("Please do not access this file directly. Thanks!<br/><a href='https://www.mapsmarker.com/go'>www.mapsmarker.com</a>"); }
		...
		if (!empty($marker))
		{
			...
			if ( (isset($lmm_options[ 'defaults_marker_panel_directions' ] ) == TRUE ) && ( $lmm_options[ 'defaults_marker_panel_directions' ] == 1 ) ) {
					if ($lmm_options['directions_provider'] == 'googlemaps') {
						...
						$lmm_out .= '<a href="http://www.bing.com/maps/default.aspx?v=2&amp;rtp=pos___e_~pos.' . $lat . '_' . $lon . $bing_to .'" target="_blank" title="' . esc_attr__('Get directions','lmm') . '"><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-car.png" width="14" height="14" class="lmm-panel-api-images" alt="' . esc_attr__('Get directions','lmm') . '" /></a>';
					}
			}
	...
	$table_name_markers = $wpdb->prefix.'leafletmapsmarker_markers';
	$table_name_layers = $wpdb->prefix.'leafletmapsmarker_layers';
	$layer_mark_list_microformats = $wpdb->get_results($wpdb->prepare('SELECT l.id as lid,l.name as lname, m.lon as mlon, m.lat as mlat, m.markername as markername,m.id as markerid FROM `'.$table_name_layers.'` as l INNER JOIN `'.$table_name_markers.'` AS m ON l.id=m.layer WHERE l.id = %d LIMIT 250',$layer), ARRAY_A);
	...
	foreach ($layer_marker_list as $row){
		...
		if ( (isset($lmm_options[ 'defaults_layer_listmarkers_api_directions' ] ) == TRUE ) && ( $lmm_options[ 'defaults_layer_listmarkers_api_directions' ] == 1 ) ) {
			if ($lmm_options['directions_provider'] == 'googlemaps') {
				...
				$lmm_out .= '<a href="http://www.bing.com/maps/default.aspx?v=2&rtp=pos___e_~pos.' . $row['mlat'] . '_' . $row['mlon'] . $bing_to . '" target="_blank" title="' . esc_attr__('Get directions','lmm') . '"><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-car.png" width="14" height="14" class="lmm-panel-api-images" alt="' . esc_attr__('Get directions','lmm') . '" /></a>';
			}
		}
		...
		if ( (isset($lmm_options[ 'defaults_layer_listmarkers_show_markername' ]) == TRUE ) && ($lmm_options[ 'defaults_layer_listmarkers_show_markername' ] == 1 ) ) {
			if ($lmm_options['defaults_layer_listmarkers_link_action'] != 'disabled') {
			$lmm_out .= '<strong><a title="' . esc_attr__('show marker on map','lmm') . '" href="#lmm_'.$uid.'" onclick="javascript:listmarkers_action_' . $uid . '(' . $row['markerid'] . ')">' . stripslashes(htmlspecialchars($row['markername'])) . '</a></strong>';
			} else {
   ...
   if ($markercount > $lmm_options[ 'defaults_layer_listmarkers_limit' ]) {
	...
	if ($lmm_options['directions_popuptext_panel'] == 'yes') {
		...
		if (!empty($layer)) {
			...
			if ($lmm_options['async_geojson_loading'] == 'enabled') {
				...
				$lmmjs_out .= 'xhReq_'.$mapname_js.'.onload = function xhReq_'.$uid.'(e) { if (xhReq_'.$mapname_js.'.readyState === 4) { if (xhReq_'.$mapname_js.'.status === 200) {'.PHP_EOL; //info: async 1/2			
	...
	if (processed === total) {
		...
		if ($lmm_options['async_geojson_loading'] == 'enabled') {
			$lmmjs_out .= '} else { console.error(xhReq_'.$mapname_js.'.statusText); } } }; xhReq_'.$mapname_js.'.onerror = function xhReq_'.$uid.'(e) { console.error(xhReq_'.$mapname_js.'.statusText); }; xhReq_'.$mapname_js.'.send(null);'.PHP_EOL; //info: async 2/2
		}
	...
	if ( ((empty($layer) && empty($marker))) || (!empty($marker)) ) {
		if ($lmm_options['defaults_marker_popups_add_markername'] == 'true') {
			if ($markername != NULL) {
				$markername_popup_hidden = '<div class="popup-markername"  style="border-bottom:1px solid #f0f0e7;padding-bottom:5px;margin-bottom:6px;">' . strip_tags(htmlspecialchars_decode($markername)) . '</div>';
			} else {
		...
		$lmm_out .= "<div style='display:none' id='" . $mapname . "-popuptext-hidden'>".$markername_popup_hidden.do_shortcode($mpopuptext_sanitized)."</div>";
	}
