					...
					for ($row = 2; $row <= $highestRow; ++$row) {
						...
						**********************************/
						if ($marker_id_check == NULL) {
							...
							$markername = str_replace("\"","'", $objWorksheet->getCellByColumnAndRow(1, $row)); //info: double quotes break maps; backslash not supported
					...
					for ($row = 2; $row <= $highestRow; ++$row) {
						...
						**********************************/
						if ($layer_id_check == NULL) {
							...
							$name = str_replace("\"","'", $objWorksheet->getCellByColumnAndRow(1, $row)); //info: double quotes break maps; backslash not supported
							//info: prepare address
							if ($objWorksheet->getCellByColumnAndRow(2, $row)->getDataType() != 'null') {
								$address = $objWorksheet->getCellByColumnAndRow(2, $row);
							} else {
								$address = '';
							}
