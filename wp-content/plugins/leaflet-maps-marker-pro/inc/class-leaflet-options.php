	...
	*/
	...
	public function save_defaults_for_new_options() {
		...
		if (get_option('leafletmapsmarker_version') == '3.9.4' )
		{
			...
			foreach ( $this->settings as $id => $setting )
			{
				if ( $setting['type'] != 'heading' && $setting['type'] != 'helptext' && $setting['type'] != 'helptext-twocolumn' && $setting['version'] == '3.10')
				{
				$new_options_defaults[$id] = $setting['std'];
				}
			}
		...
		*/
		...
		if (get_option('leafletmapsmarker_version_pro') == '1.9.2' )
		{
			...
			foreach ( $this->settings as $id => $setting )
			{
				if ( $setting['type'] != 'heading' && $setting['type'] != 'helptext' && $setting['type'] != 'helptext-twocolumn' && $setting['version'] == 'p2.0')
				{
				$new_options_defaults[$id] = $setting['std'];
				}
			}
