...
if ( ($action == 'add') || ($action == 'edit') || ($action == 'duplicate') ) {
	...
	if ( $error_message == null ) { //info: dont show if get error
		if ( lmm_validate_license_info() ) {
			...
			if ( $latest_pro_version_transient === FALSE ) {
				$latest_pro_version_array = wp_remote_get('https://www.mapsmarker.com/version.json', array( 'sslverify' => false, 'timeout' => 10 ));
	...
	if (get_option('leafletmapsmarker_update_info') == 'show') {
		...
		$lmm_changelog_new_version = '<a href="https://www.mapsmarker.com/v' . $lmm_version_new . 'p" target="_blank" style="text-decoration:none;">https://www.mapsmarker.com/v' . $lmm_version_new . 'p</a>';
