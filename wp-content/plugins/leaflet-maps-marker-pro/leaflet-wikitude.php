		...
		if ($searchterm != NULL) {
			...
			if ($layer == '*' or $layer == 'all') {
				...
				if ($mlm_check == 0) {
					...
					if (count($checkedlayers) > 0) {
						$q = "WHERE m.layer IN (" . implode(",", $checkedlayers) . ") and m.lat BETWEEN " . $boundingBoxLatitude1 . " AND " . $boundingBoxLatitude2 . " AND m.lon BETWEEN " . $boundingBoxLongitude1 . " AND " . $boundingBoxLongitude2 . "";
					}
				} else if ( ($mlm_check == 1) && (!in_array('all',$mlm_check_list) ) ) {
					$clayer = 0;
					$q = "WHERE m.layer IN (" . implode(",", $mlm_check_list) . ") and m.lat BETWEEN " . $boundingBoxLatitude1 . " AND " . $boundingBoxLatitude2 . " AND m.lon BETWEEN " . $boundingBoxLongitude1 . " AND " . $boundingBoxLongitude2 . "";
				} else if ( ($mlm_check == 1) && (in_array('all',$mlm_check_list) ) ) {
					$clayer = 0;
					$q = '';
				}
