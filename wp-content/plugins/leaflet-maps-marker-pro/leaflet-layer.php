	...
	if ($layerviewlat === NULL) {
		...
		<?php 
			...
			if (lmm_check_capability_edit($lcreatedby) == TRUE) {
				...
				echo '<input id="submit_top" style="font-weight:bold;margin-left:10px;" type="submit" name="layer" class="submit button-primary" value="' . $button_text . '" />';
			} else {
				if ($isedit === true) { 
							...
							foreach ($layer_marker_list as $row) {
								...
								if ( (isset($lmm_options[ 'defaults_layer_listmarkers_api_directions' ] ) == TRUE ) && ( $lmm_options[ 'defaults_layer_listmarkers_api_directions' ] == 1 ) ) {
									if ($lmm_options['directions_provider'] == 'googlemaps') {
										...
										echo '<a tabindex="130" href="http://www.bing.com/maps/default.aspx?v=2&rtp=pos___e_~pos.' . $row['mlat'] . '_' . $row['mlon'] . $bing_to . '" target="_blank" title="' . esc_attr__('Get directions','lmm') . '"><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-car.png" width="14" height="14" class="lmm-panel-api-images" alt="' . esc_attr__('Get directions','lmm') . '" /></a>';
									}
								}
								...
								if ( (isset($lmm_options[ 'defaults_layer_listmarkers_show_markername' ]) == TRUE ) && ($lmm_options[ 'defaults_layer_listmarkers_show_markername' ] == 1 ) ) {
									if ($lmm_options['defaults_layer_listmarkers_link_action'] != 'disabled') {
										echo '<strong><a title="' . esc_attr__('show marker on map','lmm') . '" href="#address" onclick="javascript:listmarkers_action(' . $row['markerid'] . ')">' . stripslashes(htmlspecialchars($row['markername'])) . '</a></strong> (<a title="' . esc_attr__('Edit marker','lmm') . ' (ID ' . $row['markerid'].')" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_marker&id=' . $row['markerid'].'">' . __('edit','lmm') . '</a>)';
									} else { 
		...
		<?php
			...
			if (lmm_check_capability_edit($lcreatedby) == TRUE) {
				if ($isedit === true) { $button_text = __('update','lmm'); } else { $button_text = __('publish','lmm'); }
				echo '<input id="submit_bottom" style="font-weight:bold;' . $edit_button_css . '" type="submit" name="layer" class="submit button-primary" value="' . $button_text . '" />';
			} else {
				if ($isedit === true) { 
		...
		<td>
			...
			if (lmm_check_capability_delete($lcreatedby) == TRUE) {
				...
				echo '<input id="delete" class="submit button-secondary lmm-nav-secondary" style="color:#FF0000;" type="submit" name="layer" value="' . __('delete', 'lmm') . '" onclick="return confirm(\'' . $confirm . '\')" />';
				echo '</div></form>';
			} else {
			...
			if (lmm_check_capability_delete($lcreatedby) == TRUE) {
				...
				if ($multi_layer_map == 0) {
					echo "<input id='delete_layer_and_markers' class='submit button-secondary lmm-nav-secondary' style='color:#FF0000;margin-left:40px;' type='submit' name='layer' value='" . __('delete layer AND assigned markers', 'lmm') . "' onclick='return confirm(\"".$confirm2 ."\")' />";
				}
				echo '</div></form>';
			} else {
